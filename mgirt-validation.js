...
// ✅ Normalize names (lowercase, trim, handle "&")
...
document.getElementById("mgIrtFile").addEventListener("change", (event) => {
...
function runMgIrtValidation(diFile, mgIrtData, resultsContainer) {
  Papa.parse(diFile, {
    ...
    complete: (r) => {
      ...
      // ✅ Build lookup map for MG/IRT names
      const mgMap = new Map();
      ...
      // ✅ Compare DI rows against MG/IRT map
      diData.forEach((diRow) => {
        ...
        // Run validation if AT LEAST ONE of MG or IRT is empty
